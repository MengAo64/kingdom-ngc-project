...
func UpdateSnipingToken() {
	...
	if global.SnipingToken != lastSnipingToken {
		...
		logger.Info("New claim token!", logger.FieldString("old", global.HideTokenLog(lastSnipingToken)), logger.FieldString("new", global.HideTokenLog(global.SnipingToken)))
	}
}
...
func main() {
	...
	// TODO: initially the sniper gets from the API, but obviously
	// there is no API access here, so you'll have to change it
	global.DetectedNitros = []string{}

	global.Hostname, err = os.Hostname()
	if err != nil {
		global.Hostname = "Iancu"
		err = nil
	}
	...
	// have a cron job that'll update the discord build number every ~12 hours
	go func() {
		scheduler := gocron.NewScheduler(time.Local)
		scheduler.Every(12).Hours().Do(func() {
			if buildNumber, err := sniper.GetDiscordBuildNumber(); err == nil {
				global.DiscordBuildNumber = buildNumber
			}
		})
	...
	// goroutine: send stats to the API every 30 seconds
	go func() {
		for !global.ShouldKill {
			time.Sleep(time.Second * 30)

			// apiStatsMessages := atomic.LoadUint64(&global.APIStatsMessages)
			// apiStatsInvites := atomic.LoadUint64(&global.APIStatsInvites)
			global.APIStatsMessages = 0
			global.APIStatsInvites = 0

			// TODO: if you want to send stats to your API
			// implement here (uncomment apiStatsMessages & apiStatsInvites and send those)
			// if you don't want to, simply comment this whole goroutine as it's useless
		}
	}()
	...
	if global.Config.Sniper.SaveInvites {
		...
		logger.Warn("I just want to warn you that you are not saving the invites that the sniper is detecting!")
	}
